...
list1.forEach(id => 
  webMail.push(`${id.email}, ${id.website}`)
  );
...
const user9IncompleteTasks = list2.filter (id => id.completed === false);
...
const user5posts = list3.filter (user => user.userId === 5);

console.log(user5posts);
//then, create a list of title and body pairs
const titlePairs = list3.map(id => `${id.title} - ${id.body}`);
